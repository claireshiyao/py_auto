...
class TestLogin(unittest.TestCase):
    ...
    def test_login(self, test_data):
        ...
        try:
            ...
            excel_handler.write(config.data_path, 'login', test_data['id'] + 1, 10, 'PASS')
        except AssertionError as e:
            print("断言失败，用例不通过")
            excel_handler.write(config.data_path, 'login', test_data['id'] + 1, 10, 'FAIL')
            raise e
        finally:
            excel_handler.write(config.data_path, 'login', test_data['id'] + 1, 9, f'{res}')
