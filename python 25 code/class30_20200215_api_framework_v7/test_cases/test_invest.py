...
class TestRecharge(unittest.TestCase):
    ...
    def test_recharge(self, test_data):
        ...
        invest_sum = self.db.query('select sum(amount) from invest where loan_id = %s;', args=[loan_id, ])
        loan_amount = self.db.query('select amount from invest where id = %s;', args=[loan_id, ])
        above_loan_amount = loan_amount - invest_sum + 10

        if "#above_loan_amount#" in test_data['data']:
            test_data['data'] = test_data['data'].replace("#above_leave_amount#", str(above_loan_amount))
