...
class BasePage:
    ...
    def switch_to_alert(self, loc, image_name):
        logging.info("在{}等待alert元素{}存在并切换到alert".format(image_name, loc))
        try:
            WebDriverWait(self.driver, 20).until(EC.alert_is_present(loc))
        except:
            self.save_page_shot(image_name)
    ...
    def switch_to_new_window(self, image_name):
        logging.info("在{}等待新窗口出现并切换到新窗口".format(image_name))
        time.sleep(2)
        wins = self.driver.window_handlers
        try:
            self.driver.switch_to.window(wins[-1])
        except:
            self.save_page_shot(image_name)
            logging.exception("等待新窗口出现失败")
            raise

    def execute_script(self, js, image_name):
        logging.info("准备执行js脚本")
        try:
            self.driver.execute_script(js)
        except:
            self.save_page_shot(image_name)
            logging.exception("执行js脚本失败")
            raise
